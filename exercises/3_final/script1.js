...
class ObjectInfo {
    ...
    constructor(translation, rotation, uniforms, vertexArray) {
...
class Model {
    ...
    constructor(gl, vs, fs, bufferInfo, uniforms = 0, objectsNumber = 0, translationDist = [0, 0, 0], rotationDist = [0, 0, 0], initialTranslation = [0, 0, 0], initialRotation = [0, 0, 0]) {
        ...
        let vertexArray = twgl.createVAOFromBufferInfo(gl, this.programInfo, bufferInfo);

        for(let i = 0; i < objectsNumber; i++) {

            this.objects[i] = new ObjectInfo(currentTranslation, currentRotation, new Uniforms([0.5, 0.5, 0.5, 1]), vertexArray);
    ...
    draw() {
        ...
        for(let i = 0; i < this.objects.length; i++) {
            ...
            this.gl.bindVertexArray(this.objects[i].vertexArray);
